...
import ComposeYaml from "./components/ComposeYaml";
import ComposeBuilder from "./components/ComposeBuilder";
import ComposeOrchestration from "./components/ComposeOrchestration";
import EnvironmentVariables from "./components/EnvironmentVariables";
import { cn } from "@/lib/utils";
import { useAnimationStore } from "@/stores/animationStore";

const STEPS = [
  { id: 0, label: "Compose YAML Editor" },
  { id: 1, label: "Compose Stack Builder" },
  { id: 2, label: "Service Orchestration" },
  { id: 3, label: "Environment Variables Sync" }
];
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-h-0 bg-transparent">
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        {activeStep === 0 && <ComposeYaml />}
        {activeStep === 1 && <ComposeBuilder />}
        {activeStep === 2 && <ComposeOrchestration />}
        {activeStep === 3 && <EnvironmentVariables />}
      </div>
